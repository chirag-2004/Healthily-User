...
function Header() {
  ...
  const [healthifyId, setHealthifyId] = useState("");
  ...
  const handleMockLogout = () => {
    setUser(null);
    setHealthifyId(""); // Clear HealthifyId on logout
  };
  // --- End Mock User State ---
  ...
  useEffect(() => {
    // Fetch HealthifyId if user is logged in and has an email
    if (user && user.email) {
      axios
        .post(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/getHealthifyId`, {
          email: user.email,
        })
        .then((res) => {
          setHealthifyId(res.data.healthifyId);
        })
        .catch((error) => {
          setHealthifyId("Not Found");
          console.error("Failed to fetch HealthifyId:", error.response?.data?.message || error.message);
        });
    } else {
      setHealthifyId(""); // Clear HealthifyId if no user or no email
    }
    ...
    <nav className="p-3 flex justify-between bg-white items-center shadow-sm text-black sticky top-0 z-50">
      <Link href="/" className="flex items-center gap-2">
        <Image
          ...
          alt="logo"
        />
        <span className="text-lg font-medium font-display">Healthify</span>
      </Link>
          ...
          <Popover>
            ...
            <PopoverContent className="w-56 mr-4 mt-1"> {/* Adjusted width and margin */}
              <div className="p-2">
                ...
                {healthifyId && <p className="text-xs text-gray-500 mb-2">HealthifyId: {healthifyId}</p>}
                <hr className="my-2"/>
        ...
        <ul className="flex flex-col gap-2 font-body font-medium p-3">
          ...
          {!user ? (
            ...
            <>
              ...
              <li className="p-3 text-sm text-gray-500">HealthifyId: {healthifyId || "N/A"}</li>
